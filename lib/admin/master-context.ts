...
export const MASTER_CONTEXT_LAST_UPDATED_ISO = "2026-05-11T13:23:08.078Z"
...
- **Phase 63.5 — Billing invoice defaults control:** \`/settings/billing\` passed \`orgPermissions.canEditOrgBilling\` into \`WorkspaceInvoiceDefaultsCard\`, but \`useOrgPermissions()\` only exposes capabilities via \`.has(key)\` / \`.permissions\` — the flag was always \`undefined\`, so the payment-terms \`<select>\` stayed disabled. Fixed to \`status === "ready" && has("canEditOrgBilling")\` (matches PATCH gate on \`/api/.../billing/default-invoice-terms\`). **UX:** save button idle label is now "No changes to save" instead of a misleading always-on "Saved".
