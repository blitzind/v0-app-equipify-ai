...
/** Sample tools: owners + admins only (not managers), plus platform admins. */
export const DEMO_DATA_ADMIN_ROLES = new Set(["owner", "admin"])

export function isValidOrganizationId(id: string): boolean {
  return UUID_RE.test(id.trim())
}

export type DemoDataGate =
  | { ok: true; organizationId: string; platformAdmin: boolean; userId: string }
  | { ok: false; status: number; message: string }

export async function gateDemoDataManagement(
  supabase: SupabaseClient,
  user: User | null,
  organizationIdRaw: string | null,
): Promise<DemoDataGate> {
  if (!user) {
  ...
  if (!DEMO_DATA_ADMIN_ROLES.has(role)) {
    return {
      ok: false,
      status: 403,
      message: "Only workspace owners and admins can manage sample data.",
    }
  }
