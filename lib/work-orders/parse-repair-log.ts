...
function normalizeTaskEntry(raw: unknown, index: number): { id: string; label: string; done: boolean } | null {
  ...
  return { id, label: label.trim(), done }
}
...
export function parseRepairLog(raw: unknown): RepairLog {
  ...
  if (Array.isArray(o.tasks)) {
    tasks = o.tasks
      ...
      .filter((x): x is { id: string; label: string; done: boolean } => x !== null)
  }
  return {
