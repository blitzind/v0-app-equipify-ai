...
import { parseInvoicePreviewPayloadFromPreparedAction } from "@/lib/aiden/actions/executors/create-invoice-from-work-order-executor"
import type {
...
const MAX_ABS_UNIT_CENTS = 100_000_000 // $1M unit sanity bound

const ALLOWED_KINDS = new Set<string>(["labor", "parts", "materials", "fee", "manual"])
...
function parseIncomingLineItems(raw: unknown): { ok: true; lines: CreateInvoicePreviewLineItem[] } | { ok: false; message: string } {
  ...
  const lines: CreateInvoicePreviewLineItem[] = []
  for (const item of raw) {
    ...
    let source: CreateInvoicePreviewLineItem["source"]
    if (kind === "manual") {
      source = "manual"
    ...
    lines.push({
      kind: kind as CreateInvoicePreviewLineItem["kind"],
      description,
      quantity,
      unitCents,
      lineTotalCents,
...
/**
 * Merges a client-supplied invoice preview patch into the stored preview.
 * Immutable: `customer`, `workOrder`, `taxEstimate`, `warnings`, `recommendedInvoiceTitle`, `sourceSummary`.
 * Validates financial fields and recomputes subtotal/total from line items + existing tax.
 */
export function mergeAndValidateInvoicePreviewForPatch(
  existingPreviewPayload: Record<string, unknown>,
  incomingBody: unknown,
):
  | { ok: true; previewPayload: Record<string, unknown> }
  | { ok: false; message: string } {
  const base = parseInvoicePreviewPayloadFromPreparedAction(existingPreviewPayload)
  if (!base.ok) {
  ...
  const linesParsed = parseIncomingLineItems(incomingPreview.lineItems)
  ...
  const tax = p0.taxEstimate
  const totalMajor = tax == null ? subtotalMajor : subtotalMajor + tax

  if (!Number.isFinite(subtotalMajor) || !Number.isFinite(totalMajor)) {
    return { ok: false, message: "Computed totals are not finite." }
  }

  const merged: CreateInvoiceFromWorkOrderPreviewPayload = {
    ...
    recommendedInvoiceTitle: p0.recommendedInvoiceTitle,
    sourceSummary: p0.sourceSummary,
  }

  const previewPayload = { preview: merged }
  const roundTrip = parseInvoicePreviewPayloadFromPreparedAction(previewPayload as Record<string, unknown>)
  if (!roundTrip.ok) {
