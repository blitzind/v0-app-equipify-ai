...
export function buildIntentPixelScript(options: GrowthIntentPixelScriptOptions): string {
  ...
  var CONSENT_KEY="equipify_intent_consent";
  ...
  function consent(){try{var s=localStorage.getItem(CONSENT_KEY);return s||${defaultConsent}}catch(e){return ${defaultConsent}}}
  ...
  w.EquipifyIntentPixel={
    ...
    pageview:function(){PAGE_START=Date.now();send("pageview")}
  };
  send("pageview");
