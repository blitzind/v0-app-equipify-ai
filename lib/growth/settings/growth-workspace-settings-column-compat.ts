...
export const GROWTH_ORGANIZATION_AI_TEAMMATE_ACTIVATION_COLUMNS = [
  "autonomous_activated_at",
  "autonomous_activated_by_user_id",
] as const

export function isGrowthOrganizationAiTeammateActivationColumnMissingError(
  error: { message?: string; code?: string } | null | undefined,
): boolean {
  if (!error) return false
  const message = typeof error.message === "string" ? error.message : String(error)
  ...
  if (code === "42703" || code === "PGRST204") {
    return GROWTH_ORGANIZATION_AI_TEAMMATE_ACTIVATION_COLUMNS.some((column) =>
