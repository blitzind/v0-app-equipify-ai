/** Phase 7.PS-IP — Benchmark officer/principal discovery orchestrator. Server-only. */

import "server-only"

import type { SupabaseClient } from "@supabase/supabase-js"
import { acquireBenchmarkOfficerPrincipalEvidence } from "@/lib/growth/benchmark/apollo-replacement-benchmark-officer-principal-acquisition"
import { reconcileBenchmarkOfficerPrincipalEvidence } from "@/lib/growth/benchmark/apollo-replacement-benchmark-officer-principal-reconciliation"
import {
  GROWTH_APOLLO_REPLACEMENT_BENCHMARK_OFFICER_PRINCIPAL_QA_MARKER,
  type BenchmarkOfficerPrincipalDiscoveryMetrics,
...
function emptyMetrics(): BenchmarkOfficerPrincipalDiscoveryMetrics {
  return {
    companies_queried: 0,
    evidence_sources_queried: 0,
    sources_with_records: 0,
    officer_records_found: 0,
    principal_records_found: 0,
...
export async function runApolloReplacementBenchmarkOfficerPrincipalDiscovery(
  admin: SupabaseClient,
): Promise<{
  qa_marker: typeof GROWTH_APOLLO_REPLACEMENT_BENCHMARK_OFFICER_PRINCIPAL_QA_MARKER
  ok: boolean
  companies_queried: number
  evidence_sources_queried: Array<{ key: string; label: string; source_type: string; company_name: string }>
  ...
  metrics: BenchmarkOfficerPrincipalDiscoveryMetrics
  messages: string[]
}> {
  ...
  metrics.companies_queried = cohortCompanies.length
  messages.push(`benchmark_cohort=${cohortCompanies.length} companies`)

  const acquisition = await acquireBenchmarkOfficerPrincipalEvidence({
    cohort: cohortCompanies.map((c) => ({
  ...
  metrics.evidence_sources_queried = acquisition.sources_queried.length
  metrics.sources_with_records = acquisition.sources_with_records
  metrics.officer_records_found = acquisition.officer_records_found
  metrics.principal_records_found = acquisition.principal_records_found
  metrics.evidence_records_collected = acquisition.records.length
  messages.push(...acquisition.messages)

  const reconciliation = await reconcileBenchmarkOfficerPrincipalEvidence(admin, {
  ...
  if (
    ...
    messages.push("committee: skipped — no new officer/principal evidence")
  }

  const ok =
    metrics.companies_queried > 0 &&
  ...
  return {
    qa_marker: GROWTH_APOLLO_REPLACEMENT_BENCHMARK_OFFICER_PRINCIPAL_QA_MARKER,
    ok,
    companies_queried: metrics.companies_queried,
    evidence_sources_queried: acquisition.sources_queried.map((s) => ({
      key: s.key,
      label: s.label,
      source_type: s.source_type,
      company_name: s.company_name,
    })),
    officer_records_found: metrics.officer_records_found,
    principal_records_found: metrics.principal_records_found,
    evidence_records_collected: metrics.evidence_records_collected,
