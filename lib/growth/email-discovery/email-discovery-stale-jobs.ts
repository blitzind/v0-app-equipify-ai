...
export type RecoverStaleEmailDiscoveryJobsInput = {
  company_id?: string
  person_id?: string
 ...
 * Idempotent: only updates rows still `running` with started_at (or updated_at) before cutoff.
 * Does not insert jobs — dedupe unchanged; operators may enqueue a new pending job after recovery.
 */
export async function recoverStaleEmailDiscoveryRunningJobs(
  admin: SupabaseClient,
  input: RecoverStaleEmailDiscoveryJobsInput = {},
): Promise<{ recovered: number }> {
  const stale_after_ms = input.stale_after_ms ?? GROWTH_EMAIL_DISCOVERY_STALE_RUNNING_MS
  ...
  const last_error = GROWTH_EMAIL_DISCOVERY_STALE_RUNNING_ERROR
