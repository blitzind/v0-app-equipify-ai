...
import {
  ...
  queueEmailDiscoveryForCompanyRoles,
} from "@/lib/growth/email-discovery/email-discovery-queue"

/** Trigger 2 — after canonical person + role persisted; non-blocking. */
export async function triggerEmailDiscoveryAfterPersonPersist(
  ...
  try {
    await enqueueEmailDiscoveryJob(admin, {
      company_id,
      person_id,
...
export async function triggerEmailDiscoveryAfterCompanyEnriched(
  admin: SupabaseClient,
  ...
  try {
    await queueEmailDiscoveryForCompanyRoles(admin, {
      company_id,
