/**
 * S5-B — production/integration Supabase bootstrap for automation builder certification.
 */
...
export type GrowthAutomationBuilderCertBootstrapResult = GrowthOperatorNotificationsCertBootstrapResult

export function bootstrapGrowthAutomationBuilderCertEnv(input?: {
  requireVercelProductionEnvRun?: boolean
}): GrowthAutomationBuilderCertBootstrapResult | null {
  return bootstrapGrowthOperatorNotificationsCertEnv(input)
}

export function describeAutomationBuilderCertBootstrapFailure(input?: {
  requireVercelProductionEnvRun?: boolean
  ...
  if (requireVercelProductionEnvRun && !vercelProductionEnvRun) {
    return {
      ...
      note:
        "Run via pnpm test:growth-automation-builder:production with vercel-production-env-run.ts (.env.local not used)",
