...
async function loadCompanyContactRow(
  admin: SupabaseClient,
...
async function resolveOrCreateLeadForEnrollmentCandidate(
  admin: SupabaseClient,
  input: {
  ...
  const contact = await loadCompanyContactRow(admin, input.company_contact_id)
  ...
  const lead = await createGrowthLead(admin, {
    sourceKind: "other",
    sourceDetail: "apollo_enrollment_automation",
    externalRef: normalized.externalRef,
...
export async function runApolloEnrollmentAutoEnrollmentForCompany(
  ...
  for (const contact of targets) {
    ...
    const companyContact = contact.company_contact_id
      ? await loadCompanyContactRow(admin, contact.company_contact_id)
      : null
