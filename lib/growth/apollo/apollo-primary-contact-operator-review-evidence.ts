...
function buildReviewRowFromCandidate(input: {
  ...
  return {
    ...
    operator_review_status: readApolloOperatorReviewStatus(metadata),
    outreach_ready: readApolloOperatorOutreachReady(metadata),
    blockers: buildApolloContactReviewBlockers(pseudoRow),
    contact_status: "candidate",
...
export function mergeApolloOperatorReviewRows(input: {
  ...
  const rows: ApolloPrimaryContactOperatorReviewRow[] = []
  const linkedCandidateIds = new Set<string>()

  for (const contact of input.company_contacts) {
    if (!isApolloSourcedCompanyContactRow(contact)) continue
    const candidateId = asString(contact.contact_candidate_id)
    if (candidateId) linkedCandidateIds.add(candidateId)
    rows.push(buildReviewRowFromCompanyContact({ row: contact, company_name: input.company_name }))
  }
  ...
  for (const candidate of input.contact_candidates) {
    ...
    rows.push(buildReviewRowFromCandidate({ row: candidate, company_name: input.company_name }))
  }
...
export function buildApolloPrimaryContactOperatorReviewSnapshot(input: {
  ...
  const contacts = mergeApolloOperatorReviewRows({
  ...
  return {
    ...
    summary: {
      ...
      contactable: contacts.filter((row) => row.contactable).length,
      sequence_ready: contacts.filter((row) => row.sequence_ready).length,
