/**
 * S1-B — production/integration Supabase bootstrap for share page templates certification.
...
export type GrowthSharePageTemplatesCertBootstrapResult = GrowthOperatorNotificationsCertBootstrapResult

export function bootstrapGrowthSharePageTemplatesCertEnv(input?: {
  requireVercelProductionEnvRun?: boolean
}): GrowthSharePageTemplatesCertBootstrapResult | null {
  return bootstrapGrowthOperatorNotificationsCertEnv(input)
}

export function describeSharePageTemplatesCertBootstrapFailure(input?: {
  requireVercelProductionEnvRun?: boolean
  ...
  if (requireVercelProductionEnvRun && !vercelProductionEnvRun) {
    return {
      ...
      note:
        "Run via pnpm test:growth-share-page-templates:production with vercel-production-env-run.ts (.env.local not used)",
  ...
  if (!projectRef) {
    return {
      ...
      note:
        "Link Supabase CLI to production (supabase link --project-ref) or set SUPABASE_PROJECT_REF",
