...
function mapDomain(row: Record<string, unknown>): GrowthSenderDomain {
  return {
    ...
    tracking_domain: asString(row.tracking_domain) || null,
    domain_age_note: asString(row.domain_age_note) || null,
    manual_override: Boolean(row.manual_override),
    verification_source: (asString(row.verification_source) || "stub") as GrowthSenderDomain["verification_source"],
    last_verified_at: asString(row.last_verified_at) || null,
    verification_error: asString(row.verification_error) || null,
