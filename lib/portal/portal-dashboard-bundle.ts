...
export type PortalDashboardBundle = {
  stats: {
    equipmentTotal: number
    equipmentDueSoon: number
    openWorkOrders: number
    outstandingCents: number
    /** Non-paid invoice rows (sent + overdue, etc.) for dashboard-style summaries. */
  ...
  certificateSummary: { total: number; unlocked: number; locked: number }
}

/**
 * Core portal dashboard payload for one org + customer (same rules as `/api/portal/dashboard`).
 * Callers must enforce auth (portal session or trusted staff preview).
 */
export async function fetchPortalDashboardBundle(
  svc: SupabaseClient,
  organizationId: string,
  customerId: string,
): Promise<PortalDashboardBundle> {
  ...
  const unpaidInvoiceCount = (invAll.data ?? []).filter((i) => i.status !== "paid").length
  ...
  const [{ data: nextAppointmentRows }, { data: recentCompletedRows }] = await Promise.all([
    svc
      .from("work_orders")
      .select(
        "id, work_order_number, title, status, scheduled_on, scheduled_time, assigned_user_id, assigned_technician_id, equipment_id",
      )
      .eq("organization_id", orgId)
      .eq("customer_id", custId)
      .eq("is_archived", false)
  ...
  return {
    ...
    nextAppointment: nextAppointment
      ? {
          ...
          statusLabel: mapCustomerWorkOrderStatus(
            nextAppointment.status as string,
            nextAppointment.scheduled_on as string | null,
          ),
          ...
          technicianName:
            nextAppointment.assigned_technician_id &&
            appointmentTechMap.get(nextAppointment.assigned_technician_id as string)
    ...
    recentCompletedService: recentCompleted
      ? {
          ...
          statusLabel: mapCustomerWorkOrderStatus(
            recentCompleted.status as string,
            recentCompleted.scheduled_on as string | null,
          ),
