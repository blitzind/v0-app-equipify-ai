...
import { GROWTH_REVENUE_2A_HOTFIX_4_QA_MARKER } from "@/lib/growth/draft-factory/draft-factory-admission-downstream-reconcile-2a"
...
async function main(): Promise<void> {
  ...
  const ticksToClearBacklog =
    blockingViolations > 0
      ...
      : 0
  console.log("\n--- Reconciliation Throughput (HOTFIX-1/2/3/4) ---")
  console.log(`HOTFIX-4 QA marker: ${GROWTH_REVENUE_2A_HOTFIX_4_QA_MARKER}`)
  ...
  console.log(
    "Expected after HOTFIX-4 deploy + one scheduler tick: attempted ~= remaining violations, corrected ~= attempted, budget_exhausted_phase null in draft_factory_due_scheduler_tick logs.",
  )
