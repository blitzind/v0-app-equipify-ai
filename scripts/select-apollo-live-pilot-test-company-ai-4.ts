 ...
 * Run: pnpm select:apollo-live-pilot-test-company-ai-4:production
 *
 * Prefer a seeded company by domain:
 ...
 *   APOLLO_TEST_COMPANY_NAME=<name>          — alias for name search
 *   APOLLO_TEST_COMPANY_PROFILE=henry_schein — preset domain/name for selection
...
async function main(): Promise<void> {
  ...
  const result = await resolveApolloLivePilotTestCompany(admin, selection)
