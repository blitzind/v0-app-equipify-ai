...
assert.ok(settingsPanel.includes('data-qa-section="ai-teammate-settings"'))
assert.ok(settingsPanel.includes("growthAvaSettingsTitle"))
assert.ok(settingsPanel.includes("Role"))
assert.ok(settingsPanel.includes("Currently unavailable"))
...
assert.ok(onboarding.includes("ai-teammate-onboarding"))
console.log("  ✓ onboarding dialog introduces default teammate identity")

const identityModule = readSource("lib/workspace/ai-teammate-identity.ts")
assert.ok(identityModule.includes("@fuzor/identity"))
