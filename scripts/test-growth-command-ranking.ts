...
const toolsGroup = GROWTH_NAV_GROUP_DEFS.find((group) => group.id === "tools")
assert.ok(toolsGroup?.items.some((item) => item.label === "Imports"))
assert.ok(toolsGroup?.items.some((item) => item.label === "Playbooks"))

const communicationGroup = GROWTH_NAV_GROUP_DEFS.find((group) => group.id === "communication")
assert.equal(communicationGroup?.label, "Communication")
assert.ok(communicationGroup?.items.some((item) => item.label === "Reply Inbox"))
assert.ok(communicationGroup?.items.some((item) => item.label === "Call Queue"))

const coachingGroup = GROWTH_NAV_GROUP_DEFS.find((group) => group.id === "coaching")
assert.equal(coachingGroup?.label, "Coaching")
assert.ok(coachingGroup?.items.some((item) => item.label === "Live Coaching"))
assert.ok(coachingGroup?.items.some((item) => item.label === "Call Providers"))

const workflowGroup = GROWTH_NAV_GROUP_DEFS.find((group) => group.id === "workflow")
assert.ok(!workflowGroup?.items.some((item) => item.label === "Reply Inbox"))
assert.ok(!workflowGroup?.items.some((item) => item.label === "Meetings"))
