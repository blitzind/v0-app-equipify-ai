...
assert.ok(packet.personalization.some((line) => /nationwide depot/i.test(line)))
...
assert.ok(card.includes("Research evidence"))
assert.ok(card.includes("Sales strategy brief"))
assert.ok(card.includes("Risk panel"))
assert.ok(card.includes("Transparency"))
