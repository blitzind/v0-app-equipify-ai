/**
 * AVA-DISCOVERY-SEARCH-DIVERSITY-AND-EXHAUSTION-1A — Production observability probe (read-only by default).
 *
 *   pnpm probe:ava-discovery-search-diversity-and-exhaustion-1a:production
 *
 * Bounded scheduler portfolio tick (same path as cron, no approval/send):
 *   AVA_DISCOVERY_SEARCH_DIVERSITY_AND_EXHAUSTION_1A_EXECUTE=true pnpm probe:ava-discovery-search-diversity-and-exhaustion-1a:production
 */
...
async function main(): Promise<void> {
  console.log(`[${CERT_ID}] discovery search diversity audit`)
  ...
  const execute = process.env.AVA_DISCOVERY_SEARCH_DIVERSITY_AND_EXHAUSTION_1A_EXECUTE === "true"
    ...
    JSON.stringify(
      {
        ...
        before: {
          newCompaniesVelocity: before.newCompaniesVelocity,
          discoverySearchSlice: before.discoverySearchSlice,
          lastSuccessfulDataMoonRun: before.lastSuccessfulDataMoonRun,
          lastNewCompanyIngested: before.lastNewCompanyIngested,
        },
        portfolioTick,
        after: {
          newCompaniesVelocity: after.newCompaniesVelocity,
          discoverySearchSlice: after.discoverySearchSlice,
          lastSuccessfulDataMoonRun: after.lastSuccessfulDataMoonRun,
          lastNewCompanyIngested: after.lastNewCompanyIngested,
        },
        invariants: {
