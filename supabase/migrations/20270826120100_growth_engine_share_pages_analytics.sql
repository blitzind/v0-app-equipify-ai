-- Growth Engine SR-2B-3 — Share page analytics timeline, signals, attribution columns
...
-- -----------------------------------------------------------------------------
-- High-intent share page signal types
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Attribution engagement score columns for share page activity
-- -----------------------------------------------------------------------------

alter table growth.engagement_scores
  add column if not exists page_views integer not null default 0 check (page_views >= 0);

alter table growth.engagement_scores
  add column if not exists page_engaged integer not null default 0 check (page_engaged >= 0);

alter table growth.engagement_scores
  add column if not exists page_cta_clicks integer not null default 0 check (page_cta_clicks >= 0);

alter table growth.engagement_scores
  add column if not exists page_bookings_completed integer not null default 0 check (page_bookings_completed >= 0);
