-- GS-SENDR-2E — SENDR public engagement timeline event types (repo parity with TypeScript timeline-types)

do $$
begin
  if to_regclass('growth.lead_timeline_events') is null then
    raise exception 'SENDR timeline migration requires growth.lead_timeline_events';
  end if;
end $$;
...
alter table growth.lead_timeline_events
  add constraint lead_timeline_events_event_type_check check (event_type in (
    ...
    'landing_page_viewed', 'video_started', 'video_completed',
    'cta_clicked', 'booking_started', 'booking_completed'
  ));

comment on column growth.lead_timeline_events.event_type is
  'GS-SENDR-2E adds SENDR public engagement timeline types (landing_page_viewed, video_*, cta_clicked, booking_*).';
