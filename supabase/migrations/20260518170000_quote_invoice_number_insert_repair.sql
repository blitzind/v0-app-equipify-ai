-- Repair migration: guarantee quote/invoice number generation on insert.
-- Safe to run repeatedly across drifted environments.
...
-- Backfill existing missing values.
...
-- Make inserts safe even when frontend omits number fields.
alter table public.org_quotes
  alter column quote_number set default public.format_quote_number(nextval('public.org_quote_number_seq'));

alter table public.org_invoices
  alter column invoice_number set default public.format_invoice_number(nextval('public.org_invoice_number_seq'));

-- Keep trigger fallback in case explicit blank strings are inserted.
