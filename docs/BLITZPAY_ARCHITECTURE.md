...
Design document for BlitzPay / Connect. **Phase 1** (`docs/BLITZPAY_PHASE_1.md`) implements **Stripe Connect Express onboarding**. **Phases 2A–2J** (`docs/BLITZPAY_PHASE_2_ARCHITECTURE.md` §12) add hosted **Stripe Checkout** for **org invoices** on the **connected account** (staff dashboard + **customer portal**), `application_fee_amount`, webhook-driven `org_invoice_payments`, fee snapshots, ledger rows, **staff refunds**, **dispute visibility**, diagnostics APIs, **platform-managed** convenience-fee disclosure, payout/balance-transaction ledger + reconciliation (staff only), **Phase 2I** multi-method payment foundations (card + ACH) with stored profile references (Stripe-hosted details only), and **Phase 2J** collections automation foundations (reminders, recovery states, hosted payment links, read-only collection insights/reporting). **Phase 2O** adds **provider-agnostic financing session/offers**, **installment / staged payment plans**, org-level **revenue acceleration** toggles, and **read-only insights** (no Equipify-side lending or autonomous credit decisions) — see `docs/BLITZPAY_PHASE_2_ARCHITECTURE.md` §12.16. **Phase 2P** adds a **work order BlitzPay panel**, **`canAssistBlitzpayCollection`** field-safe pay-link and checkout actions, optional **WO ↔ payment plan** linkage, **reporting** for work-order-originated links and field “invoice later” markers — see §12.17. **Phase 2Q** adds **revenue intelligence** (dashboard + forecasts + collections metrics + read-only recommendations) for financial roles and **platform revenue rollup** — see §12.18. **Phase 2T** adds the **financial command center** (owner scorecards + combined AR/AP cash outlook) — see §12.21. **Phase 2U** adds **deterministic executive business health** (bounded WO/invoice sampling, customer payment signals, platform health rollup) — **no LLM** — see §12.22. **Phase 2V** adds **collections copilot + cash acceleration metrics** (priority queue, playbooks, field/ACH/installment heuristics, platform collections rollup) — **deterministic only, staff + platform admin** — see §12.23. **Phase 2W** adds **recurring revenue / membership renewal** signals (maintenance plans, service agreements, scheduled renewals, installment cadence, bounded MRR-style proxies, platform recurring rollup) — **deterministic, staff + platform admin, no portal admin** — see §12.24. **Phase 2X** adds **native BlitzPay memberships** (Postgres tables, recurring invoice generation with idempotency, failure retries, retention snapshots, cron tick), **minimal portal read APIs** (no raw Stripe identifiers in payloads), and **staff dashboards** — see §12.25; it complements Phase 2W heuristics without replacing Connect invoice payment flows. **Phase 2Y** adds **payroll-style accruals** (technician commissions, WO-linked **contractor settlements** distinct from Phase 2S `blitzpay_vendor_payouts`, revenue-share rules/ledger, payroll run orchestration — **accounting only, no ACH payroll**) — see §12.26. **Phase 2Z** adds **internal cash-bucket planning** (`blitzpay_cash_accounts`, allocations, reserve rules, runway snapshots), deterministic operating-cash and reserve-gap estimates layered on treasury + AP + payroll signals — **not bank accounts, not custodial balances, no money transmission**; Stripe Connect remains the movement source of truth — see §12.27. **Phase 2AA** (**Phase 2 extension — shipped**) adds **customer billing profiles**, **saved payment-method metadata** (hashed Stripe references + masked brand/last4 only), **autopay enrollment preferences**, bounded reporting rates, and **minimal portal billing APIs** — **no PAN/bank numbers, no raw Stripe ids in client payloads, no charges or subscription creation in this phase** — see `docs/BLITZPAY_PHASE_2_ARCHITECTURE.md` §12.28. **Phase 2AB** (**Phase 2 extension — shipped**) adds **invoice collection state**, **deterministic capped retry scheduling** (day 1 / 3 / 7 / 14 + final review), **recovery-flow + activity-log orchestration** (no autonomous email/SMS, no off-Stripe charges), **staff collections APIs**, **portal billing overview** (`/portal/billing`), and reporting snapshot collection KPIs — **not** enterprise “Phase 3B — Native AP Automation & Bill Pay”; it remains Connect-centric collections orchestration — see §12.29. **Phase 3A (internal general ledger — shipped)** adds double-entry COA, journals, periods, and deferred-revenue foundations — see `docs/BLITZPAY_PHASE_3_ARCHITECTURE.md`. **Phase 3B (native vendor AP & bill pay foundations — shipped)** adds **vendors**, **vendor bills/lines**, **approval-flow rows**, **payment-run + allocation orchestration** (planning only; **no autonomous money movement**), **append-only AP audit events**, **GL accrual on approve**, **paid-bill DB immutability**, and **bounded** AP health / reporting fields — same doc (Phase 3B section). **Phase 3C (tax & compliance foundations — shipped)** adds **tax jurisdictions & rules**, **deterministic tax calculation rows**, **immutable compliance audit log**, **ACH authorization retention** (hashed references), **vendor tax / 1099 readiness profiles**, **tax liability snapshots**, **bounded** tax/compliance reporting + staff UI — **no filing/remittance, no legal guarantees, no portal compliance APIs** — see same doc (Phase 3C section). **Phase 3D (financing marketplace foundations — shipped)** adds **marketplace provider registry** (`blitzpay_marketplace_financing_providers`, distinct from Phase **2O** catalog `blitzpay_financing_providers`), **financing applications**, **application offers** (`blitzpay_financing_application_offers`), **contractor advance planning models**, **append-only financing audit**, **provider match rows**, **portal-safe financing summaries**, **bounded** financing reporting + FCC tiles — **no lending/custody, no autonomous underwriting** — see same doc (Phase 3D section). **Phase 3E (procurement & inventory finance — shipped)** adds **inventory financial items/movements**, **valuation snapshots**, **vendor rebate programs + accruals** (accrual tracking only), **reorder forecasts** (planning only), **serialized asset financials** (hashed serial references), **append-only procurement audit**, **bounded** procurement reporting + FCC tiles — **no autonomous purchasing or reorder execution** — see same doc (Phase 3E section). **Historical migration filenames** still contain `phase_3a` / `phase_3b` / `phase_3c` / `phase_3d` / `phase_3e` for applied-migration integrity; **product/docs labels** use **Phase 2AA** / **Phase 2AB** for those files, **Phase 3A** for the GL migration `20261010120000_blitzpay_phase_3a_general_ledger.sql`, **Phase 3C** for `20261013120000_blitzpay_phase_3c_tax_compliance.sql`, **Phase 3D** for `20261014120000_blitzpay_phase_3d_financing_marketplace.sql`, and **Phase 3E** for `20261015120000_blitzpay_phase_3e_procurement_inventory_finance.sql`.

**BlitzPay roadmap series:** **Phase 2** (2A–2Z plus **2AA** / **2AB**) = Stripe-backed payments, billing, treasury visibility, payroll/payout, cash planning, recurring/membership signals, customer billing + saved payment metadata + autopay foundations (**2AA**), and deterministic invoice collections orchestration (**2AB**) — still movement- and Connect-centric. **Phase 3A (general ledger — shipped)** = native **double-entry internal books** (chart of accounts, journals, periods, deferred revenue foundations, bounded reporting) — see `docs/BLITZPAY_PHASE_3_ARCHITECTURE.md`. **Phase 3B (vendor AP foundations — shipped)** = deterministic **vendor bill lifecycle**, **treasury-aware scheduling signals**, **approval thresholds**, and **reconciliation-ready** allocations — **orchestration only**; Stripe remains the movement path for Connect payouts when you pay through approved flows — see same doc. **Phase 3C (tax & compliance foundations — shipped)** = **jurisdictions + rules**, **deterministic calculations**, **append-only compliance audit**, **ACH retention tracking**, **1099 readiness**, **bounded** reporting + FCC tiles — **no automated filing/remittance** — see `docs/BLITZPAY_PHASE_3_ARCHITECTURE.md` (Phase 3C). **Phase 3D (financing marketplace foundations — shipped)** = **marketplace providers + applications + offers + matches**, **contractor advance models**, **append-only financing audit**, **portal financing summaries**, **bounded** reporting + FCC — **no direct lending** — see same doc (Phase 3D). **Phase 3E (procurement & inventory finance — shipped)** = **inventory valuation + movement ledger**, **rebate accruals**, **reorder forecasting**, **serialized asset hashes**, **procurement audit**, **bounded** reporting + FCC — **no autonomous purchasing** — see same doc (Phase 3E). **Phase 4A (AI financial copilot — shipped)** adds **advisory** Postgres artifacts (`blitzpay_ai_financial_insights`, `blitzpay_ai_recommendation_actions`, `blitzpay_ai_forecast_snapshots`, append-only `blitzpay_ai_audit_log`), deterministic scoring helpers, org-scoped **staff** APIs under `/blitzpay/ai/*`, and FCC / Insights hub UI — **no autonomous money movement** — see `docs/BLITZPAY_PHASE_4_ARCHITECTURE.md`. **Phase 4B (revenue optimization foundations — shipped)** adds deterministic **opportunity + action queues**, **payment behavior scores** (bounded reads), **experiment tracking**, append-only **revenue optimization audit**, staff APIs under `/blitzpay/revenue-optimization/*`, reporting snapshot fields, and FCC / Insights panels — **no autonomous customer outreach, pricing, membership changes, financing approvals, or payment retries beyond existing Phase 2AB caps** — see the same doc. **Phase 5A (multi-entity / franchise finance foundations — shipped)** adds explicit **financial groups + membership linkage**, **inter-company balance tracking** (no autonomous settlement), **consolidated KPI snapshots** (reporting-only), **shared operational benchmarks**, append-only **multi-entity audit** with deterministic hashes, **bounded** org-scoped APIs under `/blitzpay/multi-entity/*`, reporting snapshot + FCC fields, and Insights / FCC UI — **no merged org databases, no cross-org visibility without linkage + finance roles, no automatic GL consolidation** — see `docs/BLITZPAY_PHASE_5_ARCHITECTURE.md`. **Phase 5B (supplier / vendor network foundations — shipped)** adds opt-in **supplier networks**, **membership**, **preferred pricing / bulk / financing visibility** rows, **aggregate-only** shared procurement benchmarks, **org-local** supplier performance scores, append-only **supplier network audit**, bounded staff APIs under `/blitzpay/supplier-network/*`, eight reporting snapshot + FCC fields — **no autonomous procurement, pricing negotiation, financing execution, cross-org customer data, or marketplace trading** — see the same doc. **Phase 5C (warranty / claims / protection foundations — shipped)** adds warranty **reserve** tracking, **claims** workflow rows, **protection plans**, **storm financial** planning rows, **payout tracking** with opaque reference hashes, append-only **claims audit**, **GL claims account placeholders**, bounded staff APIs under `/blitzpay/claims/*`, `/blitzpay/protection-plans`, `/blitzpay/storm-events`, eight reporting snapshot + FCC fields, and Insights / FCC **Claims & protection** UI — **no underwriting, no autonomous claim adjudication or payouts, no insurer role, no legal warranty guarantees, no cross-org claim PII** — see the same doc. **Phase 6A (mobile financial ops foundations — shipped)** adds **offline-safe intent capture**, **hashed signature authorization references**, **mobile payroll approval queue rows**, **field treasury snapshot bands**, **sync batch metadata**, append-only **mobile audit** (`immutable_hash`), org-scoped staff APIs under `/blitzpay/mobile/*`, eight reporting snapshot + FCC fields, and **Mobile financial ops** UI — **no offline money movement, no GL/AP/tax authority from mobile, no raw payment credentials or raw signatures in list APIs** — see `docs/BLITZPAY_PHASE_6_ARCHITECTURE.md`. **Phase 6B (enterprise scale & observability foundations — shipped)** adds **financial event metadata**, **workflow execution tracking**, **queue health snapshots**, **idempotency records**, append-only **observability audit**, **multi-region readiness metadata**, bounded staff **`/blitzpay/observability/*` APIs**, **manual replay visibility** (owner/admin/platform; **no autonomous execution**), eight reporting snapshot + FCC fields, **Enterprise observability** UI, and platform **`/api/platform/blitzpay/observability-rollup`** — **metrics and orchestration visibility only; no duplicate money movement; no hidden worker automation** — see `docs/BLITZPAY_PHASE_6_ARCHITECTURE.md` (Phase 6B). **Phase 7A.5 (demo & showcase foundations)** expands deterministic **demo org archetypes**, **FCC-style showcase snapshots**, **humanized activity narratives**, and **mixed health tones** in `lib/blitzpay/blitzpay-demo-presets.ts` for sales screenshots and onboarding polish — **no tenant isolation changes, no production PII, no DB bulk seeds, no financial math changes** — see `docs/BLITZPAY_PHASE_7_ARCHITECTURE.md` §11. **Phase 7A.6 (Stripe live readiness & webhook safety)** expands **`blitzpay-stripe-readiness-guards.ts`**, adds an additive **Stripe live readiness** strip on the FCC payload, and enriches **platform BlitzPay Ops** summaries with bounded webhook/Connect posture signals and **safe** load failures — **no autonomous Stripe remediation, no new payment products** — see `docs/BLITZPAY_PHASE_7_ARCHITECTURE.md` §12. **Phase 7A.7 (commercial packaging & positioning infrastructure)** centralizes **module packaging categories**, **onboarding and operational-maturity histogram helpers**, **upgrade recommendation copy**, **sales triads**, and a bounded **`commercialPackagingHistogram`** on platform BlitzPay ops (normalized `plan_id` sample only; **no organization identifiers**); `BlitzpayPlanAwarenessStrip` adds a subtle secondary positioning line on staff surfaces — **no Stripe subscription enforcement, no paywalls, no checkout** — see `docs/BLITZPAY_PHASE_7_ARCHITECTURE.md` §13. **Phase 5** = network/ecosystem expansion. **Phase 6** = mobile/scale/observability maturity.
...
- [BLITZPAY_PHASE_7_ARCHITECTURE.md](./BLITZPAY_PHASE_7_ARCHITECTURE.md) — **Phase 7A / 7A.2:** production hardening, UX cleanup, reporting nesting cap, **BlitzPay feature catalog + plan/packaging metadata** (permissive access by default; optional `enforceTierGates` for future server guards), soft plan-awareness UI on key staff surfaces, FCC operational readiness, Stripe readiness helpers (no new movement products; entitlement helpers remain Stripe-subscription-decoupled).
