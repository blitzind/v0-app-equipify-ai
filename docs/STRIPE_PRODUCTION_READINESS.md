...
### BlitzPay Connect (Phase 7A supplement)

Use this as a **staff checklist** alongside Phase 1–2 webhook docs — not a substitute for Stripe Dashboard configuration.

- [ ] **Webhook:** `POST /api/blitzpay/webhook` uses `STRIPE_BLITZPAY_WEBHOOK_SECRET` and persists `evt_…` ids in `blitzpay_stripe_webhook_events` before side effects (replay-safe).
- [ ] **Connect onboarding:** Treat `account.updated` as authoritative; keep requirements arrays staff-visible only; retest disconnect/reconnect by creating a fresh test Express account when changing business profile fields.
- [ ] **Payouts / disputes / ACH:** rely on existing Phase **2H** payout ledger sync + **2E** dispute/refund handlers; confirm `payout.*` events are enabled for connected accounts in the environment under test.
- [ ] **Payload hygiene:** customer portal and staff list APIs must not return raw `acct_`, `cus_`, `pi_`, or `pm_` tokens — use `lib/blitzpay/blitzpay-stripe-readiness-guards.ts` `scanJsonForStripeLikeTokens` in QA scripts when reviewing new JSON shapes.
- [ ] **Phase 7A.3 payload hygiene:** observability event/idempotency list APIs and claims payout APIs use `lib/blitzpay/blitzpay-payload-sanitization.ts`; portal invoice/quote `POST …/blitzpay/prepare-pay` success returns **`{ url }` only** for Checkout redirect; platform observability rollup uses safe JSON on load failure. Re-run `pnpm test:blitzpay-phase-7a3-security-hardening` when changing those JSON shapes.
- [ ] **Phase 7A architecture:** [BLITZPAY_PHASE_7_ARCHITECTURE.md](./BLITZPAY_PHASE_7_ARCHITECTURE.md) — nesting caps, entitlement hooks, operational readiness, demo presets (7A.5), **Stripe live readiness strip + platform ops summaries (7A.6)**.
- [ ] **Phase 7A.6 live readiness:** run `pnpm test:blitzpay-phase-7a6-stripe-live-readiness`; confirm FCC **Stripe live readiness** card appears for finance roles; confirm platform **BlitzPay Ops** shows host API mode + webhook secret flag + bounded webhook inbox / Connect posture counts.
