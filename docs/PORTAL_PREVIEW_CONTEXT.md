...
4. `GET /api/portal/bootstrap` supplies shell data (names, logo from `pickPreferredDocumentLogoUrl`, customer company).
5. Portal pages fetch their data via org-scoped APIs using the portal cookie.
...
| Branding accent | Global CSS defaults unless overridden | **`organizations.primary_color`** applied as inline `--portal-accent*` (same tokens as live portal) |
...
- **Accent color:** `organizations.primary_color` → `portalAccentCssVariables()` (`lib/portal/portal-theme-css.ts`) on the preview root element.
...
`GET /api/portal/public-branding?organizationId=` returns `organizationName`, `logoUrl`, and **`primaryColor`** (for future login theming or external surfaces). No secrets.
