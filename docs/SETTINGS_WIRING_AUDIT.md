...
| `/settings/notifications` | Alert matrix (in-app / email / SMS) | `organization_notification_preferences` | `GET/PATCH …/notification-preferences` | `canManageWorkspaceSettings` | **Wired** — local draft + explicit **Save notification settings** sends one PATCH (preferences + digest); SMS column visible but disabled until delivery exists; disabled reasons surfaced (permissions / view-only / DB / no workspace) |
| `/settings/notifications` | Email digest + quiet hours | `organization_notification_digest_settings` | Same Route Handler | Same | **Wired** — digest enable/frequency/time + quiet hours on same draft/save path; GET includes `quietHours` + digest aliases (`enabled` / `frequency` / `timeLocal`) alongside canonical fields; `meta.persistenceReady` gates saves |
...
- **Workspace notifications:** Alert channel matrix, digest, and quiet hours on `/settings/notifications` persist through `notification-preferences` with a **draft + explicit Save** (single PATCH bundle; see `lib/notifications/organization-notification-preferences-repository.ts`); personal channel prefs remain follow-up.
