...
| `/settings/notifications` | Alert matrix (in-app / email / SMS) | `organization_notification_preferences` | `GET/PATCH …/notification-preferences` | `canManageWorkspaceSettings` | **Wired** — optimistic UI + PATCH; SMS row disabled when org SMS delivery is off |
| `/settings/notifications` | Email digest + quiet hours | `organization_notification_digest_settings` | Same Route Handler | Same | **Wired** — digest enable/frequency/time + quiet hours; saves disabled when `meta.persistenceReady` is false (migrations missing) |
