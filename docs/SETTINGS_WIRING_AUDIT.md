...
| `/settings/workspace` | SMS notifications (policy) | `organization_sms_workspace_settings` (+ consent / suppression / delivery audit tables) | `GET/PATCH …/sms-workspace` | `canManageWorkspaceSettings` (card respects `canEdit`) | **Wired** — transactional SMS foundation; no live send until gates + provider; see `docs/SMS_ARCHITECTURE.md` |
...
| `/settings/billing` | Plan, hosted checkout, Manage billing dialog, single-step add card | Stripe + org billing fields | Billing routes + `stripe-setup` actions | `canViewBilling` / `canEditOrgBilling` for mutations | **Wired** |
...
| `/settings/notifications` | Alert matrix (in-app / email / SMS) | `organization_notification_preferences` | `GET/PATCH …/notification-preferences` | `canManageWorkspaceSettings` | **Wired** — draft + **Save**; GET includes `smsWorkspace` + `meta.smsPersistenceReady`; SMS toggles only when `smsChannelConfigurable` (workspace SMS approved path); transactional SMS UI only for **Work order completed** + **Schedule changes**; link to **Workspace** SMS card |
| `/settings/notifications` | Email digest + quiet hours | `organization_notification_digest_settings` | Same Route Handler | Same | **Wired** — digest enable/frequency/time + quiet hours on same draft/save path; quiet hours respected by SMS send helper; GET includes `quietHours` + digest aliases alongside canonical fields; `meta.persistenceReady` gates saves |
...
| `/settings/integrations` | Twilio SMS catalog entry | — | Links to `/settings/workspace#workspace-sms` | Same | **Wired** (navigation to workspace SMS policy card; carrier OAuth not in-app yet) |
...
- **Workspace notifications:** Alert channel matrix, digest, and quiet hours on `/settings/notifications` persist through `notification-preferences` with a **draft + explicit Save** (single PATCH bundle; see `lib/notifications/organization-notification-preferences-repository.ts`); **SMS** is gated by `smsWorkspace` from the same GET and workspace policy via `sms-workspace` (see `docs/SMS_ARCHITECTURE.md`); personal channel prefs remain follow-up.
