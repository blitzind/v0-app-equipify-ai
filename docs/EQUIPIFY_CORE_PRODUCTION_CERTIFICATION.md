...
| Create quote | `/quotes` | supabase | Quote with line items saved; totals correct | Critical | Untested |
| Edit quote | `/quotes` | supabase | Line item changes persist | High | Untested |
| Quote PDF | `/api/organizations/[organizationId]/quotes/[quoteId]/pdf` | supabase | PDF downloads; org branding correct | Critical | **Pass** (EC-2: route file exists; generation Untested) |
| Email quote | `/api/email/quote` | resend | Email delivered with PDF attachment | Critical | Untested |
| Quote approval (staff) | `/quotes` | supabase | Status reflects approval | High | Untested |
| Portal quote approve | `/api/portal/quotes/[quoteId]/approve` | portal_session, supabase | Customer approval recorded | Critical | Untested |
...
| Create invoice | `/invoices` | supabase | Invoice with line items saved | Critical | Untested |
| Create from work order | `/work-orders/[id]` | supabase | Line items populated from WO | Critical | Untested |
| Invoice PDF | `/api/organizations/[organizationId]/invoices/[invoiceId]/pdf` | supabase | PDF correct | Critical | **Pass** (EC-2: route file exists; generation Untested) |
| Print layout | `/organizations/[organizationId]/invoices/[invoiceId]/print` | supabase | Printable view renders | High | Untested |
| Email invoice | `/api/email/invoice` | resend | Email with PDF delivered | Critical | Untested |
...
| Feature | Route | Dependencies | Expected result | Criticality | Status |
|---------|-------|--------------|-----------------|-------------|--------|
| Stripe Connect onboarding | `/settings/payments` | stripe_connect | Express account created; status shows connected | Critical | Untested |
| Staff collect payment | `/api/organizations/[organizationId]/invoices/[invoiceId]/blitzpay/prepare-pay` | stripe_connect | Checkout completes; invoice marked paid | Critical | Untested |
...
| BlitzPay webhook | `/api/blitzpay/webhook` | stripe_connect, supabase | Charge events update invoice payment state idempotently | Critical | **Blocked** (EC-2: webhook secret not materialized locally) |
| Payment return | `/payment-return` | stripe_connect | User returned to invoice context after checkout | High | Untested |
| Receipt email | `/api/organizations/[organizationId]/invoices/[invoiceId]/blitzpay/resend-receipt` | resend, stripe_connect | Receipt email delivered after payment | High | Untested |
...
| View quotes | `/portal/quotes` | portal_session | Customer sees sent quotes | Critical | Untested |
| Approve quote | `/api/portal/quotes/[quoteId]/approve` | portal_session | Approval status updated | Critical | Untested |
| View invoices | `/portal/invoices` | portal_session | Customer sees invoices | Critical | Untested |
| Pay invoice | `/api/portal/invoices/[invoiceId]/blitzpay/prepare-pay` | portal_session, stripe_connect | Portal checkout succeeds | Critical | Untested |
...
| SaaS webhook | `/api/stripe/webhook` | stripe_saas, supabase | Subscription events sync idempotently | Critical | **Blocked** (EC-2: webhook secret not materialized locally) |
