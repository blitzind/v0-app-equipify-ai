...
import type { GrowthSocialProfileDiscoveryOperatorStatus } from "@/lib/growth/social-profile-discovery/social-profile-discovery-runtime-types"
...
type GrowthSocialProfileDiscoveryOperatorCardProps = {
  companyId: string
  personId: string
  personLabel?: string
  compact?: boolean
...
function discoveryStatusLabel(status: GrowthSocialProfileDiscoveryOperatorStatus["discovery_status"]): string {
  switch (status) {
    case "pending":
      return "Pending"
    case "running":
...
function discoveryStatusTone(
  status: GrowthSocialProfileDiscoveryOperatorStatus["discovery_status"],
...
export function GrowthSocialProfileDiscoveryOperatorCard({
  companyId,
  personId,
  personLabel,
  compact = false,
  triggerSource = "manual",
}: GrowthSocialProfileDiscoveryOperatorCardProps) {
  const [status, setStatus] = useState<GrowthSocialProfileDiscoveryOperatorStatus | null>(null)
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/platform/growth/social-profile-discovery/operator-status?${params}`, {
        cache: "no-store",
      })
      const data = (await res.json().catch(() => ({}))) as {
        ok?: boolean
        status?: GrowthSocialProfileDiscoveryOperatorStatus
        message?: string
      }
      if (!res.ok || !data.ok || !data.status) {
        throw new Error(data.message ?? "Could not load social profile discovery status.")
      }
      setStatus(data.status)
    } catch (e) {
      setStatus(null)
      setError(e instanceof Error ? e.message : "Could not load social profile discovery status.")
    } finally {
      setLoading(false)
    }
  }, [companyId, personId])
  ...
  async function enqueueDiscovery() {
    ...
    setDetail(null)
    try {
      const res = await fetch("/api/platform/growth/social-profile-discovery/jobs", {
        method: "POST",
      ...
      if (!res.ok || !data.ok) {
        throw new Error(data.message ?? "Could not queue social profile discovery.")
      }
      setMessage(
        data.enqueued
          ? "Social profile discovery queued. Results update when the worker completes."
          : data.reason === "verified_profile_exists"
            ? "Verified profile already on file."
      ...
      setError(e instanceof Error ? e.message : "Could not queue social profile discovery.")
    } finally {
      setEnqueueing(false)
    }
  }
  ...
  async function loadEvidence() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/platform/growth/social-profile-discovery/runs/${status.last_run_id}`, {
        cache: "no-store",
      })
      const data = (await res.json().catch(() => ({}))) as {
        ok?: boolean
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs font-medium text-foreground">
          {personLabel ? `Social — ${personLabel}` : "Social profile discovery"}
        </span>
        <GrowthBadge
      ...
      <dl className="grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
        <div>
          <dt className="inline">Verified profile: </dt>
          <dd className="inline font-medium text-foreground">
            {status.has_verified_profile ? status.verified_profile : "—"}
          </dd>
        </div>
        <div>
      ...
      <div className="flex flex-wrap gap-2">
        {status.can_discover ? (
          <Button size="sm" variant="outline" disabled={enqueueing} onClick={() => void enqueueDiscovery()}>
            {enqueueing ? <Loader2 className="mr-1 size-3 animate-spin" /> : <Share2 className="mr-1 size-3" />}
            Discover Social
          </Button>
        ) : null}
