...
export function GrowthCanonicalPersonBackfillPanel() {
  ...
  const [errorRows, setErrorRows] = useState<GrowthCanonicalPersonBackfillErrorRow[]>([])
  ...
  async function runBackfillChunks(mode: "dry_run" | "apply") {
    ...
    let cumulative: GrowthCanonicalPersonBackfillStats | null = null
    let cumulativeErrors: GrowthCanonicalPersonBackfillErrorRow[] = []
    let chunks = 0
    ...
    try {
      for (;;) {
        ...
        if (mode === "apply") {
          body.confirm = GROWTH_CANONICAL_PERSON_APPLY_CONFIRM
        }

        const res = await fetch("/api/platform/growth/canonical-persons/backfill", {
          method: "POST",
        ...
        cumulative = cumulative
          ? mergeCanonicalPersonBackfillStats(cumulative, data.stats)
          : data.stats
        cumulativeErrors = mergeCanonicalPersonBackfillErrorRows(
          cumulativeErrors,
          data.error_rows ?? [],
        )
  ...
  return (
    <GrowthEngineCard title="Canonical persons (7.2B)">
      <div className="space-y-4" data-qa-marker={GROWTH_CANONICAL_PERSON_QA_MARKER}>
        <p className="text-sm text-muted-foreground">
          Batched, resume-safe linkage into <code className="text-xs">growth.persons</code>. DONE only when
          verification reports <code className="text-xs">pending_total = 0</code> across contact staging tables.
        </p>
        ...
        {showApplyConfirm ? (
          <div className="rounded-lg border border-amber-200 bg-amber-50/80 p-4 dark:border-amber-900 dark:bg-amber-950/30">
            ...
            <Button
              className="mt-3"
              size="sm"
              disabled={running || confirmText !== GROWTH_CANONICAL_PERSON_APPLY_CONFIRM}
        ...
        {result?.ok ? (
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <p>
              Canonical persons in DB: {result.stats?.canonical_persons_after ?? result.summary?.canonical_persons_after ?? "—"}
              {result.pending_by_source ? (
                <>
                  {" "}
                  · candidates {result.pending_by_source.contact_candidates ?? 0} · company_contacts{" "}
                  {result.pending_by_source.company_contacts ?? 0} · decision_makers{" "}
                  {result.pending_by_source.lead_decision_makers ?? 0}
                </>
              ) : null}
            </p>
