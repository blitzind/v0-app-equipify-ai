...
function PipelineCard({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "flex w-full gap-1 rounded-lg border border-border bg-card shadow-sm transition-shadow",
        "hover:border-primary/30 hover:shadow-md",
        (isDragging || dragging) && "opacity-60 shadow-lg ring-2 ring-primary/20",
      )}
    >
      <button
        type="button"
      ...
      <button
        ...
        className="min-w-0 flex-1 rounded-r-lg p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
      ...
      <div className="mt-2 space-y-0.5 text-[11px] text-muted-foreground">
        ...
        <p className={cn("tabular-nums", fuTone)}>
          F/U: {prospect.next_follow_up_at ? formatFollowUpStamp(prospect.next_follow_up_at) : "—"}
        </p>
      </div>
      </button>
    </div>
  ...
  return (
    <div className="flex min-w-[min(100vw-2rem,280px)] max-w-[320px] shrink-0 flex-col rounded-xl border border-border bg-muted/20">
      ...
      <div
        ref={setNodeRef}
        className={cn(
          "flex min-h-[180px] flex-1 flex-col gap-2 p-2 sm:min-h-[220px]",
          isOver && canManage && "bg-primary/5 ring-1 ring-inset ring-primary/25 rounded-b-xl",
        )}
      >
        {prospects.map((p) => (
        ...
        {prospects.length === 0 ? (
          <p className="text-[11px] text-muted-foreground px-1 py-6 text-center">No prospects</p>
        ) : null}
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={null}>
        {activeProspect ? (
          <div className="w-[min(100vw-2rem,280px)] opacity-95">
