...
export function NewQuoteModal({ open, onClose, onSuccess }: NewQuoteModalProps) {
  ...
  const filteredWorkOrders = customerId
    ? workOrders.filter(wo => wo.customerId === customerId)
    : workOrders
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] flex flex-col rounded-xl border border-border bg-background shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Fill in the details below to create a quote.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <FieldSelect
                value={customerId}
                onChange={e => { setCustomerId(e.target.value); setEquipmentId(""); setWorkOrderId("") }}
              >
