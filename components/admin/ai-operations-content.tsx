...
import { Activity, Ban, Brain, DollarSign, Filter, Loader2, ShieldOff, Sparkles, XCircle } from "lucide-react"
...
export function AiOperationsContent() {
  ...
  const orgOptions = useMemo(
    () => [...accounts].sort((a, b) => (a.organizationName ?? "").localeCompare(b.organizationName ?? "", undefined, { sensitivity: "base" })),
    [accounts],
  )
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <h2 className="text-xl font-semibold text-foreground">AI usage & reliability</h2>
        </div>
      </div>
      <div className="rounded-xl border border-border bg-card p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          <div className="flex flex-col gap-1"><label className="text-[11px] text-muted-foreground">Month (UTC)</label><Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className="h-9" /></div>
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-muted-foreground">Organization</label>
            <Select value={organizationId || "__all__"} onValueChange={(v) => setOrganizationId(v === "__all__" ? "" : v)}>
              <SelectTrigger className="h-9"><SelectValue placeholder="All organizations" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="__all__">All organizations</SelectItem>
                {orgOptions.map((a) => <SelectItem key={a.id} value={a.id}>{a.name ?? a.id}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col gap-1"><label className="text-[11px] text-muted-foreground">Task (exact)</label><Input className="h-9 font-mono text-xs" value={task} onChange={(e) => setTask(e.target.value)} list="ai-op-task-hints" /><datalist id="ai-op-task-hints">{(hints?.tasks ?? []).map((t) => <option key={t} value={t} />)}</datalist></div>
          <div className="flex flex-col gap-1"><label className="text-[11px] text-muted-foreground">Outcome</label><Select value={successFilter} onValueChange={(v) => setSuccessFilter(v as "all" | "true" | "false")}><SelectTrigger className="h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All outcomes</SelectItem><SelectItem value="true">Success only</SelectItem><SelectItem value="false">Failed / blocked</SelectItem></SelectContent></Select></div>
          <div className="flex flex-col gap-1"><label className="text-[11px] text-muted-foreground">Provider (exact)</label><Input className="h-9 font-mono text-xs" value={provider} onChange={(e) => setProvider(e.target.value)} list="ai-op-provider-hints" /><datalist id="ai-op-provider-hints">{(hints?.providers ?? []).map((p) => <option key={p} value={p} />)}</datalist></div>
          <div className="flex flex-col gap-1"><label className="text-[11px] text-muted-foreground">Model</label><Input value={model} onChange={(e) => setModel(e.target.value)} className="h-9 font-mono text-xs" /></div>
        </div>
      ...
      {data ? (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            <StatCard icon={DollarSign} label="Est. cost (month)" value={fmtUsd(data.summary.totalEstimatedCostUsd)} sub={`UTC month ${data.month}`} tone="emerald" />
            <StatCard icon={Activity} label="AI log rows" value={String(data.summary.totalRequests)} sub="One row per logged event" tone="slate" />
            <StatCard icon={XCircle} label="Failed (non-budget)" value={String(data.summary.failedRequests)} tone="rose" />
            <StatCard icon={ShieldOff} label="Budget blocked" value={String(data.summary.budgetBlockedRequests)} tone="amber" />
            <StatCard icon={Ban} label="Plan blocked" value={String(data.summary.planBlockedRequests ?? 0)} sub="Subscription tier / plan limits" tone="rose" />
            ...
            <Table>
              ...
              <TableBody>
                {(data.alerts?.recentOpenAlerts ?? []).length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-muted-foreground text-sm">No open alerts.</TableCell></TableRow>
                ) : (
                  (data.alerts?.recentOpenAlerts ?? []).map((a) => (
                    <TableRow key={a.id}>
                      <TableCell className="text-xs">{a.severity}</TableCell>
                      <TableCell className="text-xs max-w-[140px] truncate">{a.organization_name ?? a.organization_id?.slice(0, 8) ?? "platform"}</TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{new Date(a.created_at).toISOString().replace("T", " ").slice(0, 19)}</TableCell>
                      <TableCell className="text-right"><div className="inline-flex gap-2"><Button size="sm" variant="secondary" onClick={() => void mutateAlert(a.id, "acknowledge")}>Acknowledge</Button><Button size="sm" variant="outline" onClick={() => void mutateAlert(a.id, "resolve")}>Resolve</Button></div></TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
            ...
            <Table>
              ...
              <TableBody>
                {data.recentLogs.length === 0 ? (
                  <TableRow><TableCell colSpan={10} className="text-muted-foreground text-sm">No rows for this month / filters.</TableCell></TableRow>
                ) : (
                  data.recentLogs.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{new Date(r.created_at).toISOString().replace("T", " ").slice(0, 19)}</TableCell>
                      <TableCell className="text-xs max-w-[140px] truncate">{r.organization_name ?? r.organization_id.slice(0, 8) + "…"}</TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground max-w-[220px] truncate">{r.failure_reason ?? "—"}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
              ...
              <Table>
                ...
                <TableBody>
                  {data.recentAiJobs.map((j) => (
                    <TableRow key={j.id}>
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{new Date(j.created_at).toISOString().replace("T", " ").slice(0, 19)}</TableCell>
                      <TableCell className="text-xs max-w-[160px] truncate">{j.organization_name ?? j.organization_id.slice(0, 8) + "…"}</TableCell>
                      <TableCell className="font-mono text-[11px]">{j.task}</TableCell>
                      <TableCell className="text-xs">{j.status}</TableCell>
                      <TableCell className="text-right tabular-nums text-xs">{Number.isFinite(j.progress_percent) ? `${j.progress_percent}%` : "—"}</TableCell>
