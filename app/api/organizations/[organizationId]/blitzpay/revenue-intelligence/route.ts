...
export async function GET(
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "GET /api/organizations/[organizationId]/blitzpay/revenue-intelligence",
  )
  ...
  try {
    const intelligence = await fetchBlitzpayOrgRevenueIntelligence(admin, organizationId, {
      reportingWindowDays: windowDays,
    })
    return NextResponse.json({ intelligence })
  } catch (e) {
    return blitzpayStaffLoadFailedResponse("GET revenue-intelligence", e)
  }
}
