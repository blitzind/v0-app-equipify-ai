...
import { reverseJournalEntry } from "@/lib/blitzpay/blitzpay-general-ledger-service"
...
export async function POST(_request: Request, context: { params: Promise<{ organizationId: string; entryId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/accounting/journal-entries/[entryId]/reverse",
  )
  ...
  try {
    const result = await reverseJournalEntry(admin, organizationId, entryId)
    return NextResponse.json(result)
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    if (msg.includes("original_not_posted")) {
      return NextResponse.json({ error: "conflict", message: msg }, { status: 409 })
    }
    return blitzpayStaffLoadFailedResponse("POST accounting/journal-entries/reverse", e)
  }
}
