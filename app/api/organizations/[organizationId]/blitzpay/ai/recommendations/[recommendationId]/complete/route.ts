...
import { completeBlitzpayAiRecommendation } from "@/lib/blitzpay/blitzpay-ai-financial-copilot"
...
export async function POST(
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/ai/recommendations/[recommendationId]/complete",
  )
  ...
  try {
    await completeBlitzpayAiRecommendation(admin, organizationId, recommendationId, {
      actorType: "user",
      actorId: gate.userId,
    })
    return NextResponse.json({ ok: true })
  } catch (e) {
    return blitzpayStaffLoadFailedResponse("POST blitzpay/ai/recommendations/complete", e)
  }
}
