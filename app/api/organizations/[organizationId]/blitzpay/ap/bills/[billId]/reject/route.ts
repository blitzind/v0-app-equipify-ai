...
import { rejectVendorBill } from "@/lib/blitzpay/blitzpay-ap-service"
...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string; billId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/ap/bills/[billId]/reject",
  )
  ...
  try {
    await rejectVendorBill(admin, organizationId, billId, gate.userId, String(body.reason ?? "rejected"))
    ...
    return blitzpayStaffLoadFailedResponse("POST blitzpay/ap/bills/reject", e)
  }
}
