...
export async function POST(
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/memberships/[membershipId]/pause",
  )
  ...
  try {
    await patchBlitzpayMembership(admin, organizationId, membershipId, { status: "paused" })
    await insertMembershipEvent(admin, {
      organizationId,
      membershipId,
      eventType: "paused",
      eventSummary: "Membership paused by staff.",
    })
    const membership = await getBlitzpayMembershipById(admin, organizationId, membershipId)
    return NextResponse.json({ membership })
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    return NextResponse.json({ error: "pause_failed", message: msg }, { status: 400 })
  }
}
