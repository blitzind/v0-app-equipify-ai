...
export async function GET(
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "GET /api/organizations/[organizationId]/blitzpay/business-health",
  )
  ...
  try {
    const businessHealth = await fetchBlitzpayBusinessHealth(admin, organizationId, {
      reportingWindowDays: windowDays,
    })
    return NextResponse.json({ businessHealth })
  } catch (e) {
    return blitzpayStaffLoadFailedResponse("GET business-health", e)
  }
}
