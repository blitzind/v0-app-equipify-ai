...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/collections/resolve",
  )
  ...
  try {
    ...
    await staffResolveCollection(admin, {
      organizationId,
      invoiceId,
      customerId,
      userId: gate.userId,
    ...
    logBlitzpayServerFailure("POST blitzpay/collections/resolve", e)
    return blitzpayStaffLoadFailedResponse("POST blitzpay/collections/resolve", e)
  }
}
