...
import { staffScheduleRetry, syncCollectionMetadataFromPaymentIntents } from "@/lib/blitzpay/blitzpay-collections-service"
...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/collections/retry",
  )
  ...
  try {
    try {
      await syncCollectionMetadataFromPaymentIntents(admin, organizationId, invoiceId)
    } catch (e) {
      logBlitzpayServerFailure("collections/retry sync metadata", e)
    }
    await staffScheduleRetry(admin, {
      organizationId,
      invoiceId,
      customerId,
      userId: gate.userId,
    ...
    if (msg.startsWith("retry_not_eligible") || (e as Error & { code?: string }).code) {
      return NextResponse.json({ error: "not_eligible", message: "A follow-up window cannot be scheduled right now." }, { status: 409 })
    }
    logBlitzpayServerFailure("POST blitzpay/collections/retry", e)
    return blitzpayStaffLoadFailedResponse("POST blitzpay/collections/retry", e)
  }
}
