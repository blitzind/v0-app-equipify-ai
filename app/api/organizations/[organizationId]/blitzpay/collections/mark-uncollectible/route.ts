...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/collections/mark-uncollectible",
  )
  ...
  try {
    ...
    await staffMarkUncollectible(admin, {
      organizationId,
      invoiceId,
      customerId,
      userId: gate.userId,
    ...
    logBlitzpayServerFailure("POST blitzpay/collections/mark-uncollectible", e)
    return blitzpayStaffLoadFailedResponse("POST blitzpay/collections/mark-uncollectible", e)
  }
}
