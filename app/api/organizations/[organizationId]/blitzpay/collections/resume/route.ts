...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  const schemaResp = await blitzpaySchemaGuardNextResponse(
    "POST /api/organizations/[organizationId]/blitzpay/collections/resume",
  )
  ...
  try {
    ...
    await staffResumeRecovery(admin, {
      organizationId,
      invoiceId,
      customerId,
      userId: gate.userId,
    ...
    logBlitzpayServerFailure("POST blitzpay/collections/resume", e)
    return blitzpayStaffLoadFailedResponse("POST blitzpay/collections/resume", e)
  }
}
