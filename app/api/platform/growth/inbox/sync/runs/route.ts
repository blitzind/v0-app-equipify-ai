...
export async function GET() {
  ...
  try {
    const runs = await enrichInboxSyncRunViews(access.admin, await listInboxSyncRuns(access.admin))
    return NextResponse.json({ ok: true, runs, privacy_note: GROWTH_INBOX_SYNC_PRIVACY_NOTE })
  } catch {
    return NextResponse.json({ error: "fetch_failed", message: "Could not load inbox sync runs." }, { status: 500 })
  }
}
