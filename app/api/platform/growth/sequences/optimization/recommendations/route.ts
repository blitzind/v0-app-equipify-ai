...
import { fetchGrowthSequenceOptimizationRecommendations } from "@/lib/growth/sequence-optimization/fetch-growth-sequence-optimization-recommendations"
...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "invalid_query", message: "Invalid optimization filters." }, { status: 400 })
  }

  try {
    const recommendations = await fetchGrowthSequenceOptimizationRecommendations(access.admin, {
