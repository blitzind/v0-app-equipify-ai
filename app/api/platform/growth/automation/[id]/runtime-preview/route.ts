...
export async function GET(
  ...
  return runRuntimePreview(request, context)
}
...
async function runRuntimePreview(
  ...
  try {
    const reconciliation = await previewAutomationRuntimeArtifacts(access.admin, {
      flowId: id,
    ...
    return NextResponse.json({
      ok: reconciliation.status === "previewed",
      reconciliation,
      artifactPreview: reconciliation.artifactPreview,
