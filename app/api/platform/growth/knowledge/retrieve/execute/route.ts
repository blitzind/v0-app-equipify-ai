...
import {
  assertKnowledgeRetrievalLayerAllowed,
  executeKnowledgeRetrievalLayerCertification,
  KNOWLEDGE_RETRIEVAL_CONFIRM,
} from "@/lib/growth/knowledge-center/knowledge-retrieval-certification"
...
export async function POST(request: Request) {
  ...
  const gateCheck = assertKnowledgeRetrievalLayerAllowed(process.env)
  if (!gateCheck.ok) {
  ...
  if (confirm !== KNOWLEDGE_RETRIEVAL_CONFIRM) {
    return NextResponse.json({ ok: false, error: "confirm_token_mismatch" }, { status: 400 })
  }

  const report = await executeKnowledgeRetrievalLayerCertification(access.admin, {
