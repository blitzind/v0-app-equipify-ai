...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "invalid_query", message: "Invalid dashboard filters." }, { status: 400 })
  }

  try {
    const dashboard = await fetchGrowthRevenueAttributionDashboard(access.admin, {
