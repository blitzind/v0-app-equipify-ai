...
import { dismissNotification } from "@/lib/growth/notifications/growth-notification-repository"
...
export async function POST(
  ...
  try {
    const updated = await dismissNotification(access.admin, id)
    if (!updated) {
    ...
    const message = error instanceof Error ? error.message : "Could not dismiss notification."
