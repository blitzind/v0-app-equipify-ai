...
import { fetchGrowthHumanExecutionDashboardView } from "@/lib/growth/human-execution/human-execution-service"
import {
...
export async function GET() {
  ...
  try {
    const dashboard = await fetchGrowthHumanExecutionDashboardView(access.admin)
    return NextResponse.json({
      ok: true,
      qaMarker: GROWTH_HUMAN_APPROVED_EXECUTION_QA_MARKER,
    ...
    const message = e instanceof Error ? e.message : "Could not load human execution dashboard."
