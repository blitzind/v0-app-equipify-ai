...
import { setVoiceParticipantHold } from "@/lib/voice/transfer-control/call-control-service"
...
export async function POST(
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, message: "Invalid hold payload." }, { status: 400 })
  }

  const result = await setVoiceParticipantHold(ctx.admin, {
