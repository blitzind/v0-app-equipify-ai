...
import { isNextBestActionEnabled } from "@/lib/growth/contact-verification/next-best-action-feature"
import {
  buildNextBestActionView,
  mapProspectSearchIntelligenceToNextBestActionInput,
} from "@/lib/growth/contact-verification/next-best-action-view"
import type { NextBestActionApiResponse } from "@/lib/growth/contact-verification/next-best-action-view"
...
function nextBestActionInputFromBody(input: z.infer<typeof BodySchema>) {
...
export async function POST(request: Request): Promise<NextResponse<NextBestActionApiResponse>> {
  if (!isNextBestActionEnabled()) {
    return NextResponse.json({ ok: false, enabled: false, message: "next_best_action_disabled" })
  }

  const access = await requireGrowthEnginePlatformAccess()
  if (!access.ok) return access.response as NextResponse<NextBestActionApiResponse>
  ...
  const prepared = nextBestActionInputFromBody(parsed.data)
  ...
  try {
    const view = await buildNextBestActionView(prepared.engineInput, { skipDns: true })

    if (!view) {
      return NextResponse.json({ ok: false, enabled: true, message: "next_best_action_unavailable" })
    }
    ...
    return NextResponse.json(
      {
        ok: false,
        enabled: true,
        message: error instanceof Error ? error.message : "next_best_action_build_failed",
      },
      { status: 500 },
    )
  }
