...
export async function GET(request: Request) {
  ...
  try {
    const payload = await getGrowthEngagementCommandCenterHighIntent(access.admin, filters)
    return NextResponse.json({ ok: true, ...payload, qa_marker: GROWTH_ENGAGEMENT_COMMAND_CENTER_QA_MARKER })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not load engagement command center high intent."
