...
export async function GET(request: Request) {
  ...
  try {
    const dashboard = await getGrowthEngagementDashboardOverview(access.admin, filters)
    return NextResponse.json({ ok: true, dashboard, qa_marker: GROWTH_ENGAGEMENT_DASHBOARD_QA_MARKER })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not load engagement dashboard."
