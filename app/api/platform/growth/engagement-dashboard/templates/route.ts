...
export async function GET(request: Request) {
  ...
  try {
    const templates = await getGrowthEngagementDashboardTemplates(access.admin, filters)
    return NextResponse.json({ ok: true, templates, qa_marker: GROWTH_ENGAGEMENT_DASHBOARD_QA_MARKER })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not load template engagement metrics."
