...
import {
  getGrowthEngagementLeadDrilldown,
...
export async function GET(request: Request, context: { params: Promise<{ leadId: string }> }) {
  ...
  try {
    const drilldown = await getGrowthEngagementLeadDrilldown(access.admin, leadId, filters)
    if (!drilldown) {
      return NextResponse.json({ ok: false, error: "not_found", message: "Lead drilldown not found." }, { status: 404 })
    }
    return NextResponse.json({ ok: true, drilldown, qa_marker: GROWTH_ENGAGEMENT_TIMELINE_QA_MARKER })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not load lead drilldown."
