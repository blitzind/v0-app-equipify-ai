...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, message: "Invalid buying committee intelligence payload." }, { status: 400 })
  }

  if (!(await isGrowthBuyingCommitteeIntelligenceRuntimeSchemaReady(access.admin))) {
    return NextResponse.json(
      { ok: false, message: "Buying committee intelligence runtime schema not ready." },
      { status: 503 },
    )
  }

  const result = await enqueueBuyingCommitteeIntelligenceJob(access.admin, {
  ...
  const status = await loadBuyingCommitteeIntelligenceOperatorStatus(access.admin, {
    company_id: parsed.data.company_id,
  })
