...
import { rejectContentTemplate } from "@/lib/growth/content/template-repository"
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!parsed.success || parsed.data.humanApprovalConfirmed !== true) {
    return NextResponse.json(
      { error: "human_approval_required", message: "Human confirmation required to reject template." },
      { status: 400 },
    )
  }
  ...
  try {
    const template = await rejectContentTemplate(access.admin, {
      templateId: id,
