...
import { completeSequenceExperiment } from "@/lib/growth/experiments/experiment-repository"
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    const experiment = await completeSequenceExperiment(access.admin, id)
    return NextResponse.json({ ok: true, experiment })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not complete experiment."
