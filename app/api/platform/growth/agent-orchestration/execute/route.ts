...
import {
  assertAgentOrchestrationExecuteAllowed,
...
export async function POST(request: Request) {
  ...
  const gateCheck = assertAgentOrchestrationExecuteAllowed(process.env)
  if (!gateCheck.ok) {
  ...
  if (confirm !== AGENT_ORCHESTRATION_CONFIRM) {
    return NextResponse.json({ ok: false, error: "confirm_token_mismatch" }, { status: 400 })
  }

  const report = await executeAgentOrchestrationCertification(access.admin, {
