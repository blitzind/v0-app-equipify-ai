...
import { rejectBusinessProfileForOrganization } from "@/lib/growth/business-profile/business-profile-service"
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const profile = await rejectBusinessProfileForOrganization(access.admin, {
      organizationId,
      profileId,
    })
    ...
    const message = error instanceof Error ? error.message : "Could not reject Business Profile."
