...
import { growthVideoOperatorWorkspaceActionSchema } from "@/lib/growth/videos/growth-video-api-schema"
import { growthVideoSafetyJson, mapGrowthVideoApiError } from "@/lib/growth/videos/growth-video-api-utils"
import { publishGrowthVideoOperatorPage } from "@/lib/growth/videos/growth-video-operator-actions-service"
import {
...
export async function POST(
  ...
  const parsed = growthVideoOperatorWorkspaceActionSchema.safeParse(await request.json().catch(() => null))
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: "invalid_body" }, { status: 400 })
  }

  try {
    const workspace = await publishGrowthVideoOperatorPage(access.admin, {
